...
public class ImageManipulator implements PlugIn {
	...
	public ImagePlus cutByMask(InputOutput.MyFileCollection mFC, ImagePlus nowTiff, ImagePlus maskTiff) {
		...
		//assign to output
		mPSA.mRSO.cubeX1 = leftX;
		mPSA.mRSO.cubeX2 = rightX;
		mPSA.mRSO.cubeY1 = topY;
		mPSA.mRSO.cubeY2 = botY;
		mPSA.mRSO.cubeZ1 = topZ;
		mPSA.mRSO.cubeZ2 = botZ;
		mPSA.mRSO.choiceOfRoi = "Cuboid";
		mPSA.mRSO.cutCanvas = true;
	
		RoiHandler.ColumnRoi colRoi = roi.prepareDesiredRoi(mFC, cutTiff, mPSA.mRSO);
	...
	public ImagePlus beamDeHardenThis(ImagePlus nowTiff, InputOutput.MyFileCollection mFC, MenuWaiter.BeamDeHardeningReturn mBDH) {
		...
		if (mBDH.isSteelColumn) {
			...
			//fit triple explonential model to radial profiles
			ObjectDetector.RadialFacts lowerRF = jOD.getRadialFacts(jCO.heightOfColumn, jCO.anglesChecked, standardRadius, radialGrayValues, mBDH.lowerQ);				
			FitStuff.FittingResults myLowerFits = jFS.fitTripleExponential(lowerRF);
			
			ObjectDetector.RadialFacts upperRF = jOD.getRadialFacts(jCO.heightOfColumn, jCO.anglesChecked, standardRadius, radialGrayValues, mBDH.upperQ);				
			FitStuff.FittingResults myUpperFits = jFS.fitTripleExponential(upperRF);
			
			outTiff = correct4SteelBeamHardening(nowTiff, jCO, lowerRF, upperRF, myLowerFits, myUpperFits, standardRadius);
			
		}
		else {
			
//			ObjectDetector.ColCoords3D jPCO = jOD.new ColCoords3D();
//			
//			int versio = jIO.checkInnerCircleFileVersion(mFC);
//			if (versio == 0) jPCO = jIO.readInnerCircleVer0(mFC);	
//			else jPCO = jIO.readInnerCircleVer1(mFC);
//			
//			outTiff = beamDeHardenThisPVC(nowTiff, jPCO, mBDH);
	...
	public ImagePlus beamDeHardenThisPVC(ImagePlus nowTiff, ObjectDetector.ColCoords3D jPCO, MenuWaiter.BeamDeHardeningReturn mBDH) {
...
//		dT.displayIP(gammaIP, "gammaIP");	
	...
	public SkeletonizerOptions addSlices4Skeletonization(ImagePlus nowTiff, MenuWaiter.PoreSpaceAnalyzerOptions mPSA, RoiHandler.ColumnRoi colRoi) {
		...
		if (mPSA.imagePhase2BeAnalyzed != 255) nowTiff = extractPhaseOfInterest(nowTiff, mPSA.imagePhase2BeAnalyzed, mPSA.nameOfAnalyzedPhase);
...
//		letsSee.updateAndDraw();letsSee.show();
	...
	public ImagePlus correct4SteelBeamHardening(ImagePlus nowTiff, ObjectDetector.EggShapedColCoords3D jCO, ObjectDetector.RadialFacts lowerRF, ObjectDetector.RadialFacts upperRF, FitStuff.FittingResults myLowerFits, FitStuff.FittingResults myUpperFits ,int standardRadius) {
		...
		for (int i = 0 ; i < nowTiff.getNSlices() ; i++) {
			...
			double rLower = lowerRF.radialProfile[i][3];
			double rUpper = upperRF.radialProfile[i][3];
			...
			for (int x = 0 ; x < nowIP.getWidth() ; x++) {
				for (int y = 0 ; y < nowIP.getHeight() ; y++) {			
					...
					if (nowRadius < radiusAtThisAngle) {
						...
						int CLower = (int)Math.round(ftotLower[renormalizedRadius]);
						int CUpper = (int)Math.round(ftotUpper[renormalizedRadius]);
						
						float mygray = nowIP.getPixelValue(x, y);
						int newgray = (int)Math.round(rLower + ((rUpper-rLower)/(CUpper-CLower)) * (mygray - CLower));
...
/*		ImagePlus test0 = new ImagePlus("S1", S1);		
	...
	public ImagePlus extractPhaseOfInterest(ImagePlus nowTiff, int myPhase, String myPhaseName) {
		...
		for (int z = 0 ; z < nowTiff.getNSlices() ; z++) {
			...
			for (int x = 0 ; x < nowTiff.getWidth() ; x++) {
				for (int y = 0 ; y < nowTiff.getHeight() ; y++) {
					...
					if (nowPix == myPhase) outIP.putPixel(x, y, 255);
					else outIP.putPixel(x, y, 0);					
				}
			}
	...
	public ImagePlus[] addColumnRegion2Stack(ImagePlus nowTiff, int maxTopDepression, int maxBotDepression, PolygonRoi[] pRoi, PolygonRoi[] iRoi) {
		...
		//decide whether using max boittom depression makes sense..
	...
	public ImagePlus calibrateGrayValues(ImagePlus nowTiff, String nowGaugePath, MenuWaiter.CalibrationReferences myNR, String myOutPath) {
		...
		if (myNR.useInnerCircle) {
			...
			if (myNR.material.equalsIgnoreCase("aluminium")) {
				...
				IJ.error("This opition is currently not implemented. Please do not use it.\nBailing now...");
				IJ.wait(1000);
				return null;
