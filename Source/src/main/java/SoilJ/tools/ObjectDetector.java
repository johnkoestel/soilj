...
import SoilJ.tools.MorphologyAnalyzer.BoneJParticles;
...
public ColCoords3D findClosestXYSlice2Bottom(InputOutput.MyFileCollection mFC, ColCoords3D samCoords, MenuWaiter.ColumnFinderMenuReturn jCFS) {
	...
	public ImagePlus makeAggregateMask(InputOutput.MyFileCollection mFC, RoiHandler.ColumnRoi colRoi, MenuWaiter.AggregateMaskOptionMenu aMO) {
		...
		double minimalParticleSize = aMO.filterSize / 100 * roiVolume;
		BoneJParticles myParty = morph.parallelParticleAnalyzer(aggTiff, minimalParticleSize, Double.POSITIVE_INFINITY);			
		aggTiff = jIM.binarizeLargerThanZero(myParty.myPartyPic);			
		ImagePlus virginTiff = aggTiff.duplicate();
		myParty.myPartyPic.killStack();
		IJ.freeMemory();IJ.freeMemory();
		...
		myParty = morph.parallelParticleAnalyzer(fuseTiff, minimalParticleSize, Double.POSITIVE_INFINITY);			
		maskTiff = jIM.binarizeLargerThanZero(myParty.myPartyPic);	
	...
	public ImagePlus goWatershed(InputOutput.MyFileCollection mFC, RoiHandler.ColumnRoi colRoi, MenuWaiter.TobiasWatershedOptionMenu aMO) {
		...
		//cut away small remnants from the dilation-watershed-erosion tribulation..
		BoneJParticles myParty = morph.parallelParticleAnalyzer(aggTiff, aMO.filterSize*roiVolume/100, Double.POSITIVE_INFINITY);			
		aggTiff = jIM.binarizeLargerThanZero(myParty.myPartyPic);	
		...
		//cut away small remnants from the dilation-watershed-erosion tribulation..
		myParty = morph.parallelParticleAnalyzer(aggTiff, aMO.filterSize*roiVolume/100, Double.POSITIVE_INFINITY);			
		aggTiff = jIM.binarizeLargerThanZero(myParty.myPartyPic);	
	...
	public ImagePlus makeClosingMask(InputOutput.MyFileCollection mFC, RoiHandler.ColumnRoi colRoi, MenuWaiter.ClosingMaskOptionMenu aMO) {
...
/*					DisplayThings disp = new DisplayThings();
	...
	public ColCoords3D findColumnWalls3D(ImagePlus nowTiff, ColCoords3D prelimCC, MenuWaiter.ColumnFinderMenuReturn jCFS, int[] sampleSlices) {
		...
		for (int i = 0 ; i < colHeight ; i++) {  
			...
			//set tiff to the correct position and get Processor etc..
			nowTiff.setPosition(i+1);
			ImageProcessor myIP = nowTiff.getProcessor().duplicate();		
			...
			//ImagePlus tesImg = new ImagePlus("", myIP);
			//tesImg.show();
			
			//apply an addition median filter		
...
/*	public ColumnContainer findColumnsWalls3D(ColumnContainer colCon) {
	...
	public ImageProcessor findSurfaceSubroutine(ImagePlus nowTiff0, int minimumObjectThickness, String topOrBottom) {
		...
		BoneJParticles myParty = mA.parallelParticleAnalyzer(inverseTop, volumeOfChunks2Exclude , Double.POSITIVE_INFINITY);	
		ImagePlus topMatrix =	myParty.myPartyPic;		
		...
		for (i = 0 ; i < topMatrix.getNSlices() ; i++) {
			...
			ImageProcessor hisIP = myIP.duplicate();
			hisIP = myIP.convertToByte(false);
			hisIP.max(1);
			hisIP.multiply(255);
	...
	public ImagePlus findSoilSurface(ImagePlus nowTiff, MenuWaiter.SurfaceFinderReturn mSFR) {
		...
		//PolygonRoi tRoi = pRoi[(int)Math.round((prelimTopIP.getMin() + prelimTopIP.getMax()) / 2)];
		//ImageProcessor smoothTopIP = smoothFoundSurface(prelimTopIP);
		//PolygonRoi bRoi = pRoi[(int)Math.round(pRoi.length - (prelimBottomIP.getMin() + prelimBottomIP.getMax()) / 2)];
...
/*		double[] xax =  new double[preciseCC.heightOfColumn];
	...
	public ImagePlus extractAirFilledPores(double tension, ImagePlus nowTiff, ImagePlus surfTiff, MenuWaiter.WRCCalculatorMenu mWRC, InputOutput.MyFileCollection mFC) {
		...
		double columnHeight = nowTiff.getNSlices() * mWRC.resolutionInMicroMeter;
		double bboundary = tension * 10000;  //calculate in micrometer
		...
		if (mWRC.choiceOfWRCType.equalsIgnoreCase("drainage")) {
			...
			for (int z = 1 ; z <= nowTiff.getNSlices() ; z++) {
				...
				nowTiff.setPosition(z);
				ImageProcessor nowIP = nowTiff.getProcessor();
				double columnNow = columnHeight - (z - 0.5) * mWRC.resolutionInMicroMeter;  //-0.5 to get to the voxel midpoint
				double cutoffRadius = capillaryConstantInMicroMeter / (bboundary + columnNow);
				double cutoffThickness = (2 * cutoffRadius) / mWRC.resolutionInMicroMeter;
		...
		int maxVol = zwischiTiff.getWidth() * zwischiTiff.getHeight() * zwischiTiff.getNSlices(); //input parameter 4 particle analyzer	
		
		//label connected clusters
		IJ.showStatus("Checking connection status...");
		Counter3D myOC = new Counter3D(zwischiTiff, 128, minVol, maxVol, false, false);
		myOC.getObjects();
		ImagePlus particleLabelTiff = myOC.getObjMap();
	
		IJ.freeMemory();IJ.freeMemory();
		
		//get label image and check it for connections to the top surface
		ArrayList<Integer> connected = null;
		if (mWRC.choiceOfWRCType.equalsIgnoreCase("drainage")) connected = mA.check4TouchingTheTop(particleLabelTiff, surfTiff);	
		...
		for (int z = 1 ; z <= nowTiff.getNSlices() ; z++) {
			...
			IJ.showStatus("Correcting for disconnected air-bubbles in slice " + z + "/" + nowTiff.getNSlices());
						
			particleLabelTiff.setPosition(z);
			ImageProcessor nowIP = particleLabelTiff.getProcessor();
			...
			for (int x = 0 ; x < outIP.getWidth() ; x++) {
				for (int y = 0 ; y < outIP.getHeight(); y++) {
					...
					if (connected.contains(nowPix)) outIP.putPixel(x, y, 255);
