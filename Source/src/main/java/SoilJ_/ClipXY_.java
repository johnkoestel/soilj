...
public class ClipXY_ extends ImagePlus implements PlugIn  {
	...
	public void run(String arg) {
		...
		MenuWaiter.ClipXYMenu clip = menu.showClipXYMenu();
		if (clip == null) return;
		...
		myPreOutFolder = "CutByMarginPlus" + clip.clipAway;
		if (clip.clipAway < 0) myPreOutFolder = "CutByMarginMinus" + (-clip.clipAway);
		...
		for (i = 0 ; i < myTiffs.length ; i++) {  //myTiffs.length
			...
			//cut image
			RoiHandler.ColumnRoi colRoi = roi.assembleInnerCircleROIs(mFC, nowTiff, true, clip.clipAway, true);	
			
			//save file
			jIO.tiffSaver(mFC.myPreOutFolder, mFC.myTiffs[i], colRoi.nowTiff);			
