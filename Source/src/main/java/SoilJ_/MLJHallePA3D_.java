 ...
 *
 * In this version, the plugin has been slightly modified to re-direct the results 
 * into an ASCII file and to suppress the default pop-up table 
 * the modification was carried out by John Koestel for the 
 * UfZ Halle soil structural database (U. Weller, S. Schlueter, H.-J. Vogel)	
 * run("MLJHallePA3D", "volume surface euler surface=[Crofton (13 dirs.)] euler=C26 path=E:/pleaseSaveItHere.txt"); 
 */

public class MLJHallePA3D_ extends ImagePlus implements PlugIn  
{
    // ====================================================
    // Global Constants
   ...
   /**
    ...
    public void run(String args) 
    {
        ...
        GenericDialog gd = new GenericDialog("MLJHallePA3D");
        ...
        gd.addCheckbox("Inertia Ellipsoid", false);
        gd.addCheckbox("Ellipsoid Elongation", false);
        gd.addCheckbox("Max. Inscribed Ball", false);
        gd.addMessage("");
        ...
        gd.addStringField("path", "E:/noname");
        gd.showDialog();
		...
		// show result
		//table.show(tableName);  this is surpressed..
		
		//.. instead save it in a structure and save it as an ASCII file
		MorphologyAnalyzer mA = new MorphologyAnalyzer();
		MorphologyAnalyzer.MLJLabellingResults labelResults = mA.new MLJLabellingResults();
		
		labelResults.volume = table.getColumnAsDoubles(0);
		labelResults.surface = 	table.getColumnAsDoubles(1);
		labelResults.euler = table.getColumnAsDoubles(2);
		
		//save it
		InputOutput jIO = new InputOutput();		
		jIO.writeMLBHallePA3DResults(path, labelResults);
