...
public class TobiasSpecialWatershedding extends ImagePlus implements PlugIn  {
	...
	public void run(String arg) {
		...
		MenuWaiter.TobiasWatershedOptionMenu aMO = menu.showTobiasWatershedOptionMenu();
		if (aMO == null) return;
		...
		for (i = 0 ; i < myTiffs.length ; i++) {  //myTiffs.length
			...
			//apply analyzes
			waterTiff = jOD.goWatershed(mFC, colRoi, aMO);			
			
			//save file
			jIO.tiffSaver(mFC.myPreOutFolder, mFC.myTiffs[i], waterTiff);			
