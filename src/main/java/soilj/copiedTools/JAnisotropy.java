 ...
 * 
 */
public class JAnisotropy implements PlugIn, DialogListener {

	public void run(String arg) {}
	
	public double autoCalculate(ImagePlus imp) {		
		if (null == imp) {
			IJ.noImage();
			return 0;
		}
		...
		/*GenericDialog gd = new GenericDialog("Anisotropy Measurement Setup");
		...
		gd.addCheckbox("Show_Plot", false);
		...
		//final boolean doAutoMode = true;
		//final boolean doSingleSphere = false;		
		final int nVectors = 50000;		
		final int minSpheres = 100;
		final int maxSpheres = 2000;
		final double tolerance = 0.005;
		final boolean doPlot = false;
		final boolean do3DResult = false;
		final boolean doAlign = false;
		final boolean doEigens = false;
