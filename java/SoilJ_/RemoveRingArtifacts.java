...
import SoilJ.tools.MenuWaiter.RingArtifactRemoverOptions;
...
public class RemoveRingArtifacts extends ImagePlus implements PlugIn  {
	...
	public void run(String arg) {
		...
		Class<?> clazz = RemoveRingArtifacts.class;
		...
		MenuWaiter.RingArtifactRemoverOptions rARO = menu.returnRingArtifactRemoverOptions();
		...
		boolean useInnerCircle = rARO.useInnerCircle;
		InputOutput.MyFolderCollection mFC = jIO.getAllMyNeededFolders(myBaseFolder, myTiffs, "", useInnerCircle, true);

		//save and create output folder		
		String myOutFolder;
		String myOutPath = null;
		myOutFolder = "RingRemoved";			
		...
		for (i = 0 ; i < myTiffs.length ; i++) {  //myTiffs.length
			...
			//select the correct inner circle if this option has been selected..	
			String innerCirclePath = "";
			if (rARO.useInnerCircle) {
				int[] myGandS = jIO.getTheCorrectGaugeNSurfaceFiles(myTiffs[i], mFC.myInnerCircleFiles, null);
				innerCirclePath =  mFC.myInnerCircleFiles[myGandS[0]];
			}
