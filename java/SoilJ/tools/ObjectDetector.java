...
import org.doube.bonej.ParticleCounter;
...
public class ObjectDetector implements PlugIn {
	...
	/*public double[][][] getRadialSteelgrayValues(ImagePlus nowTiff, ObjectDetector.ColumnCoordinates jCO, float radialMappingFactor) {
		...
		for (int i = 0 ; i < nowTiff.getNSlices() ; i++) {
			...
			for (double angle = 0 ; angle < 2 * Math.PI - Math.PI/400 ; angle = angle + 2 * Math.PI / (maxAlpha/dAlpha)) {				
				...
				float[] grayAtThisAngle = new float[(int)Math.round(nowRadius)];
				...
				SplineFitter sF = new SplineFitter(newRadius, grayAtThisAngle, radius.length);
...
public SampleBrightness getSampleBrightness(ImagePlus nowTiff, ObjectDetector.ColCoords3D jCO) {
	...
	public RadialModes getRadialPVCIllumination(ImagePlus nowTiff, ColCoords3D jCO, MenuWaiter.BeamDeHardeningReturn mBDH) {
	...
	public ColCoords3D findOrientationOfPVCOrAluColumn(ImagePlus nowTiff, MenuWaiter.ColumnFinderMenuReturn jCFS, boolean isPrecise) {
		...
		int maxAlpha = 360;
		int dAlpha = 5;		
		int checkRange = (nowTiff.getWidth()/2 + nowTiff.getHeight()/2) / 2 ;		
	...
	/*public ColumnCoordinates findAllSteelWalls(ImagePlus nowTiff, ColumnCoordinates prelimCC, double wallThickness) {
		...
		double steelgrayThreshold = 25000;
		...
		for (i = 0 ; i < nowTiff.getNSlices() ; i++) {  
			...
			for (double angle = 0 ; angle < 2 * Math.PI - Math.PI/400 ; angle = angle + 2 * Math.PI / (maxAlpha/dAlpha)) {				
				...
				for (j = 5 ; j < mgrayAtThisAngle.length ; j++) if (mgrayAtThisAngle[j] > appliedSteelgrayThreshold) {
					myWallPosition = j;
					break;
				}
				
				//if first steel was not found..
				appliedSteelgrayThreshold = steelgrayThreshold * 3 / 4;
				if (myWallPosition == 0) for (j = 0 ; j < mgrayAtThisAngle.length ; j++) if (mgrayAtThisAngle[j] > appliedSteelgrayThreshold) {
					myWallPosition = j;
					break;
				}
						
				//if first steel was not found..
				appliedSteelgrayThreshold = steelgrayThreshold * 1 / 2;
				if (myWallPosition == 0) for (j = 0 ; j < mgrayAtThisAngle.length ; j++) if (mgrayAtThisAngle[j] > appliedSteelgrayThreshold) {
					myWallPosition = j;
					break;
				}
	...
	public ColCoords3D findColumnsTop(InputOutput.MyFolderCollection mFC, ColCoords3D samCoords, MenuWaiter.ColumnFinderMenuReturn jCFS) {
		...
		int validationRange = 21; // check the topmost x slices
		...
		while (toAdd.size() < 3) {	
			...
			while (!sTW.hasConverged){  //loop until convergence
				...
				for (int i = 0 ; i < sTW.samTiff.getNSlices() ; i++) {
					...
					ImageProcessor myIP = sTW.samTiff.getProcessor();
...
public ColCoords3D findClosestXYSlice2Top(InputOutput.MyFolderCollection mFC, ColCoords3D samCoords, MenuWaiter.ColumnFinderMenuReturn jCFS) {
		
		IJ.showStatus("Finding slices near top of column ...");
		...
		//find topmost found column part in samCoords
		int soFarOnTop = jCFS.topOfColumn;
	...
	public ColCoords3D findColumnsBottom(InputOutput.MyFolderCollection mFC, ColCoords3D samCoords, MenuWaiter.ColumnFinderMenuReturn jCFS) {
		...
		ArrayList<Integer> toAdd = new ArrayList<Integer>();
		int validationRange = 21; // check the botmost x slices
		int[] valiSeries = new int[validationRange];
...
public ColCoords3D findClosestXYSlice2Bottom(InputOutput.MyFolderCollection mFC, ColCoords3D samCoords, MenuWaiter.ColumnFinderMenuReturn jCFS) {
		...
		if (botOfColumn < 2) botOfColumn = mFC.nOfSlices;
	...
	public ColCoords2D findColumnWalls2D(int sliceNum, ImageProcessor myIP, ColCoords2D prelimCC, MenuWaiter.ColumnFinderMenuReturn jCFS) {
		...
		double relativeAirWallContrast = oRelAirWallContrast;
		double rDStdThresh = oRDStdThresh;	
		...
		double rAWC_Bot = 0.5;
		double dContrast = 0.5;  //updater for relative air wall contrast
...
/*		if (jCFS.isPVC) {
		...
		double expectedCVOfWallgrayValues = 0.2; //empirically.. (formerly: colCon.jCFS.convInterCVOfWallBrightness * colCon.jCFS.coeffVarOfWallBrightness;)
		...
		while (fO == null | (R2 < jCFS.r2Thresh & trialCounter <= jCFS.maxFittingAttempts)) {
			...
			for (int j = 0 ; j < myAngle.length ; j++) {
				...
				for (int k = myWindowSize + 4 ; k < (int)Math.round(startingPoint) - myWindowSize - 6; k++) {
					...
					if (jCFS.isAlreadyNormalized) checkBool = (StatUtils.min(theNextInTheWindow) >= 0.9 * jCFS.absoluteWallgrayValue & 
							StatUtils.max(thePrevInTheWindow) <= 0.9 * jCFS.absoluteWallgrayValue);
			...
			if (R2 < jCFS.r2Thresh) {
				...
				if (meanPrelimRadius > meanFittedRadius) { //try to decrease the detection threshold..
					if (relativeAirWallContrast > rAWC_Bot) {
						...
						if (relativeAirWallContrast < rAWC_Bot) relativeAirWallContrast = rAWC_Bot;
					}						
				}
				...
				if (meanPrelimRadius < meanFittedRadius) { //try to increase the detection threshold..
					if (relativeAirWallContrast < rAWC_Top) {
						relativeAirWallContrast /= dContrast;
						if (relativeAirWallContrast > rAWC_Top) relativeAirWallContrast = rAWC_Top;
					}						
				}
...
/*					DisplayThings disp = new DisplayThings();
					...
					for (int k = startSearching4Inner ; k < stopSearching4Inner ; k++) {						
						...
						if (jCFS.showRadialProfiles & jCFS.debug) {
							...
							for (int kk = 0 ; kk < X.length ; kk++) X[kk] = (double)k;
							double[] foundPoints = new double[2];
							foundPoints[0] = locationID[j];foundPoints[1] = locationOD[j];
						
							jCFS.showRadialProfiles = dT.plotSpecialWallFinderRadii(X, mgrayAtThisAngle, dMG, foundPoints);
						}
...
/*	public ColumnContainer findColumnsWalls3D(ColumnContainer colCon) {
	...
	public ColCoords3D findColumnsBevel(InputOutput.MyFolderCollection mFC, ColCoords3D samCoords, MenuWaiter.ColumnFinderMenuReturn jCFS) {
		...
		for (int i = 0 ; i < validDataAtLocationZ.size() ; i++) {
			...
			if (outerRadiusNow < 0.99 * medianOuterRadius) {
	...
	public ImageProcessor smoothFoundSurface(ImageProcessor prelimTopIP) {
		
		HistogramStuff histo = new HistogramStuff();
		RankFilters rf = new RankFilters();
		
		int[] myHist = prelimTopIP.getHistogram();
		int myMedian = histo.findMedianFromHistogram(myHist);
		
		//init smoothSurface
		ImageProcessor smoothSurf = prelimTopIP.duplicate();
		
		//fill outside of surface
		for (int x = 0 ; x < smoothSurf.getWidth() ; x++) {
			for (int y = 0 ; y < smoothSurf.getWidth() ; y++) {
				int nowPix = smoothSurf.getPixel(x, y);
				if (nowPix == 0) {
					smoothSurf.putPixel(x, y, myMedian);				
				}
			}
		}
		...
		rf.rank(smoothSurf, 20, RankFilters.MEAN);
	...
	public ImageProcessor findSurfaceSubroutine(ImagePlus nowTiff0, PolygonRoi[] pRoi0, int minimumObjectThickness, String topOrBottom) {
		...
		ParticleCounter dPC = new ParticleCounter();
	...
	public ImagePlus findSoilSurface(ImagePlus nowTiff, String nowGaugePath, MenuWaiter.SurfaceFinderReturn mSFR) {
		...
		PolygonRoi[] pRoi = roi.makeMeAPolygonRoiStack("inner", "manual", jCO, 10);
		...
		//smooth soilSurfaceMap
		ImageProcessor smoothTopIP = smoothFoundSurface(prelimTopIP);
		ImageProcessor smoothBottomIP = smoothFoundSurface(prelimBottomIP);  
		
		topIP = smoothTopIP;
		bottomIP = smoothBottomIP;
	
		
		//transfer surface IPs to imagePlus
		outStack.addSlice(topIP);
		outStack.addSlice(bottomIP);		
...
/*		double[] xax =  new double[preciseCC.heightOfColumn];
	...
	public MorphologyAnalyzer.SurfaceStatistics extractSurfaceStatistics(InputOutput.MyFolderCollection mFC) {
		...
		int[] GandS = jIO.getTheCorrectGaugeNSurfaceFiles(mFC);
		
		//read InnerCircle file and create outline ROI		
		ColCoords3D jCO = new ColCoords3D();		
		int versio = jIO.checkInnerCircleFileVersion(mFC.myInnerCircleFiles[GandS[0]]);			
		if (versio == 0) jCO = jIO.readInnerCircleVer0(mFC.myInnerCircleFiles[GandS[0]]);	
		else jCO = jIO.readInnerCircleVer1(mFC.myInnerCircleFiles[GandS[0]]);		
		PolygonRoi[] pRoi = jRH.makeMeAPolygonRoiStack("inner", "manual", jCO, -3);
		
		//load surface file
		String mySurfaceFilePath = mFC.mySurfaceFolder + "//" + mFC.mySurfaceFileNames[GandS[1]];
	...
	public ImagePlus extractAirFilledPores(ImagePlus nowTiff, MenuWaiter.AirFilledPoresFinderMenu aPF) {
		
		double columnHeight = nowTiff.getNSlices() * aPF.resolutionInMicroMeter;
		double bboundary = aPF.matrixPotentialAtColumnBottomInCM * 10000;
		double capillaryConstantInMicroMeter = 1.48e7; 
		...
		for (int z = 1 ; z <= nowTiff.getNSlices() ; z++) {
			...
			nowTiff.setPosition(z);
			ImageProcessor nowIP = nowTiff.getProcessor();
			double columnNow = columnHeight - (z - 0.5) * aPF.resolutionInMicroMeter;
			double cutoffRadius = capillaryConstantInMicroMeter / (bboundary + columnNow);
			double cutoffThickness = (2 * cutoffRadius) / aPF.resolutionInMicroMeter;
			if (cutoffThickness < 2) cutoffThickness = Math.round(cutoffThickness);
			...
			for (int x = 0 ; x < outIP.getWidth() ; x++) {
				for (int y = 0 ; y < outIP.getHeight(); y++) {
					if (cutoffThickness < 2) outIP.putPixel(x, y, 0);
					else {
